class Hero < ActiveRecord::Base
	...
	def first_rotation
		self.date_ranges.normal_rotations.where(['start <= :start', {start: Date.today}]).order([:start, :end]).first
	end

	def last_rotation
		self.date_ranges.normal_rotations.where(['start <= :start', {start: Date.today}]).order([:end, :start]).last
	end
	...
	def rotations
		self.date_ranges.normal_rotations.where(['start <= :start', {start: Date.today}]).count
	end

	def rotations_since_newest_hero_release
		self.date_ranges.normal_rotations.where(['start <= :start', {start: Date.today}]).since_start_date(Hero.distinct_heroes.newest.release_date).count
	end

	def rotations_since_latest_change_in_roster_size
		self.date_ranges.normal_rotations.where(['start <= :start', {start: Date.today}]).since_start_date(Roster.date_range_of_latest_roster_size_change.start).count
	end


	def rotation_percentage_since_launch
		(self.rotations / DateRange.normal_rotations.where(['start <= :start', {start: Date.today}]).count.to_f) * 100
	end

	def rotation_percentage_since_release
		(self.rotations / DateRange.normal_rotations.where(['start <= :start', {start: Date.today}]).since_start_date(self.release_date).count.to_f) * 100
	end

	def rotation_percentage_since_newest_hero_release
		(self.rotations_since_newest_hero_release / DateRange.normal_rotations.where(['start <= :start', {start: Date.today}]).since_start_date(Hero.newest.release_date).count.to_f) * 100
	end

	def rotation_percentage_since_latest_change_in_roster_size
		(self.rotations_since_latest_change_in_roster_size / DateRange.normal_rotations.where(['start <= :start', {start: Date.today}]).since_start_date(Roster.date_range_of_latest_roster_size_change.start.to_date).count.to_f) * 100
	end

	def rotation_pairings
		self_date_ranges = self.date_ranges.normal_rotations.where(['start <= :start', {start: Date.today}])
		...
		for other_hero in other_heroes
			other_hero_date_ranges = other_hero.date_ranges.normal_rotations.where(['start <= :start', {start: Date.today}])
